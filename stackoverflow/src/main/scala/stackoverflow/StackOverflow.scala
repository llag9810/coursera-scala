...
class StackOverflow extends Serializable {
  ...
  def groupedPostings(postings: RDD[Posting]): RDD[(QID, Iterable[(Question, Answer)])] = {
    val questions = postings.filter(_.postingType == 1).map(p => (p.id, p)).partitionBy(new HashPartitioner(15)).persist()
    val answers = postings.filter(_.postingType == 2).map(p => (p.parentId.get, p))
    val joined = questions.leftOuterJoin(answers)
    joined.groupByKey().mapValues(_.filter(_._2.isDefined).map(pair => (pair._1, pair._2.get)))
  }
  ...
  def vectorPostings(scored: RDD[(Question, HighScore)]): RDD[(LangIndex, HighScore)] = {
    ...
    scored.map { case (question, score) => (firstLangInTag(question.tags, langs).get * langSpread, score) }.persist()
  }
  ...
  @tailrec final def kmeans(means: Array[(Int, Int)], vectors: RDD[(Int, Int)], iter: Int = 1, debug: Boolean = false): Array[(Int, Int)] = {
    ...
    val newPair = vectors.map(point => (findClosest(point, means), point))
      .groupByKey
      .map { case (mean, points) => (mean, averageVectors(points)) }
      .collect()
