...
object Anagrams {
  ...
  def wordOccurrences(w: Word): Occurrences =
    w.groupBy(x => x.toLower).map { case (ch, list) => (ch, list.length) }.toList.sortBy { case (ch, _) => ch }
  ...
  def combinations(occurrences: Occurrences): List[Occurrences] = occurrences match {
    case Nil => List(List())
    case x :: xs => {
      val restCombination = combinations(xs)
      val (ch, count) = x
      val firstCombination = (for (i <- 1 to count) yield (ch, i)).toList
      restCombination ::: firstCombination.flatMap(pair => restCombination.map(e => pair :: e))
    }
  }
  ...
  def subtract(x: Occurrences, y: Occurrences): Occurrences = {
    ...
    x.foldLeft(List[(Char, Int)]())((acc, pair) => (pair._1, pair._2 - yMap(pair._1)) :: acc)
