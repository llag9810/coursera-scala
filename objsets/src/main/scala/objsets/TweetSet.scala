...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet {
  ...
  override def mostRetweeted: Tweet = {
    if (left.isEmpty && right.isEmpty) elem
      ...
      val leftMost = left.mostRetweeted
      val rightMost = right.mostRetweeted
      if (leftMost.retweets > elem.retweets) if (leftMost.retweets > rightMost.retweets) leftMost else rightMost
      else if (elem.retweets > rightMost.retweets) elem else rightMost
    }
  }
...
object GoogleVsApple {
  ...
  def containsKeyword(list: List[String])(tweet: Tweet): Boolean =
    if (list.isEmpty) false
    else if (tweet.text.contains(list.head)) true
    else containsKeyword(list.tail)(tweet)
