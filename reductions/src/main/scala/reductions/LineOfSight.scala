...
object LineOfSight {
  ...
  def lineOfSight(input: Array[Float], output: Array[Float]): Unit = {
    ...
    while (i < input.length) {
      output(i) = if (output(i - 1) > input(i) / i) output(i - 1) else input(i) / i
      i = i + 1
    }
  }
  ...
  def downsweepSequential(input: Array[Float], output: Array[Float],
    ...
    output(from) = if (from == 0) 0 else if (input(from) / from > startingAngle) input(from) / from else startingAngle
    var i = from + 1
    while (i < until) {
      output(i) = if (output(i - 1) > input(i) / i) output(i - 1) else input(i) / i
      i = i + 1
