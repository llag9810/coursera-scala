...
package object barneshut {
  ...
  case class Leaf(centerX: Float, centerY: Float, size: Float, bodies: Seq[Body])
    ...
    val massX: Float = if (mass == 0) centerX else bodies.foldLeft(0f) { case (sum, b) => sum + b.mass * b.x } / mass: Float
    val massY: Float = if (mass == 0) centerY else bodies.foldLeft(0f) { case (sum, b) => sum + b.mass * b.y } / mass: Float
  ...
  class SectorMatrix(val boundaries: Boundaries, val sectorPrecision: Int) {
    ...
    def +=(b: Body): SectorMatrix = {
      val x = if (b.x < boundaries.minX) boundaries.minX else if (b.x > boundaries.maxX) boundaries.maxX else b.x
      val y = if (b.y < boundaries.minY) boundaries.minY else if (b.x > boundaries.maxY) boundaries.maxY else b.y
      this(((x - boundaries.minX) / sectorSize).toInt, ((y - boundaries.minY) / sectorSize).toInt) += b
      this
    }
